<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="区域" prop="area" sortable="custom" align="center" width="120" />
      ...
      <el-table-column label="操作" align="center" width="400" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          ...
          <el-button size="mini" type="success" @click="getAllFile(row)">
            查看附件
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogFileVisible" title="订单商品表" width="1000px" @close="resetShowProductList">
      <div>
        <el-link v-for="item in fileList" :key="item" :href="item">{{item}}</el-link>
      </div>
    </el-dialog>
      ...
      <el-dialog
        ...
        <el-form ref="dataForm" :rules="rules" :model="temp" >
          <el-form-item label="原因">
            ...
            <el-button type="primary" @click="checkorder(5)">
              确认
            </el-button>
        </el-form>
      </el-dialog>
      <div slot="footer" class="dialog-footer">
        <el-button @click="checkorder(6)">审核通过</el-button>
...
import { searchOrder, getFile, verifyOrder, getAllOrderByStatus, addOrder, getAllProduct, getAllDealerId } from '@/api/dealerOrder'
import { getChildCategory, getAllCategory, delDealer, startOrStopDealer, updateDealer, addChildDealer, getAllChildDealer } from '@/api/childManage'
...
export default {
  ...
  methods: {
    ...
    getAllOrder() {
      ...
      getAllOrderByStatus(4).then((response) => {
    ...
    handleFilter() {
      searchOrder(this.listQuery.dealerName, this.listQuery.dealerId, this.listQuery.orderId, this.listQuery.startAmount, this.listQuery.endAmount,4).then((response) => {
        this.orderList = response.data
        for (let i = 0; i < this.orderList.length; i++) {
          for (let j = 0; j < this.orderList[i].list.length; j++) {
            this.showProductList.push(this.orderList[i].list[j].productInfo)
