<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.name" placeholder="角色名称" style="width: 250px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="角色名称" prop="name" sortable="custom" align="center" width="200" />
      <el-table-column label="角色描述" prop="describe" sortable="custom" align="center" width="200" />
      <el-table-column label="状态" prop="status" sortable="custom" align="center" width="200" />
...
import { getAssociatedAuthority, associatedAuthority, getAllAuthority, startOrStopRole, fetchRole, addRole, updateRole, delRole, searchRole } from '@/api/myrole'
...
export default {
  ...
  methods: {
    ...
    startOrStop(flag) {
      startOrStopRole(this.multipleSelection, flag).then(() => {
    ...
    handleFilter() {
      searchRole(this.listQuery.name, this.listQuery.status).then((response) => {
        this.list = response.data
        this.total = response.data.length
        for (let i = 0; i < this.total; i++) {
          if (response.data[i].status === true) {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          addRole(this.temp.name, this.temp.describe).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateRole(tempData.id, tempData.name, tempData.describe).then(() => {
    ...
    delAllSelection() {
      ...
      delRole([this.multipleSelection]).then(() => {
        for (let i = this.list.length; i > 0; i--) {
          for (let j = 0; j < this.multipleSelection.length; j++) {
