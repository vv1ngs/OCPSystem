<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="下单日期" prop="createTime" sortable="custom" align="center" width="90" />
      ...
      <el-table-column label="订单金额" prop="totalAmount" sortable="custom" align="center" width="120" />
      <el-table-column label="区域" prop="area" sortable="custom" align="center" width="70" />
      <el-table-column label="不通过原因" prop="productOpenFare" sortable="custom" align="center" width="120" />
      <el-table-column label="状态" prop="status" sortable="custom" align="center" width="100" />
      <el-table-column label="订单体积" prop="totalVolume" sortable="custom" align="center" width="50" />
      <el-table-column label="操作" align="center" width="350" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          <el-button v-if="row.status === '未提交' || row.status=== '初次驳回请重新编辑后提交' " size="mini" type="warning" @click="handleDelete(row,$index)">
            提交订单
          </el-button>
          <el-button v-if="row.status === '未提交' || row.status=== '初次驳回请重新编辑后提交' " size="mini" type="success" @click="handleDelete(row,$index)">
            编辑订单
          </el-button>
          <el-button size="mini" type="success" @click="showProduct(row)">
            查看订单产品
          </el-button>
          <el-button v-if="row.status === '待上传附件' || row.status === '已上传附件等待复核' || row.status === '驳回等待再次上传'" size="mini" type="primary" @click="handleUpload(row)">
            上传附件
          </el-button>
          <el-button size="mini" type="danger" @click="handleDelete(row,$index)">
            取消订单
          </el-button>
          <el-button v-if="row.status === '已发货'" size="mini" type="danger" @click="handleDelete(row,$index)">
            确认收货
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="imagecropperShow" title="上传文件" width="1000px">
      <el-upload
        class="upload-demo"
        drag
        action="http://127.0.0.1:8080/dealerOrder/uploadFile/"
        :before-upload="uploadFiletest"
        :data="{'orderId':checkOrderId}"
        name="files"
        multiple>
...
import { uploadFile,getAllOrder, addOrder, getAllProduct, getAllDealerId } from '@/api/dealerOrder'
  ...
  methods: {
    ...
    uploadFiletest(file) {
      for (let i = 0; i < this.orderList.length; i++) {
        if (this.orderList[i].id === this.checkOrderId) {
          this.orderList[i].status = '已上传附件等待复核'
        }
      }
      // uploadFile(this.checkOrderId, file).then(() => {
      //
      // })
    },
    handleUpload(row) {
    ...
    handleFilter() {
      searchOrder(this.listQuery.dealerName, this.listQuery.dealerId, this.listQuery.orderId, this.listQuery.startAmount, this.listQuery.endAmount).then((response) => {
        this.orderList = response.data
        for (let i = 0; i < this.orderList.length; i++) {
          for (let j = 0; j < this.orderList[i].list.length; j++) {
            this.showProductList.push(this.orderList[i].list[j].productInfo)
